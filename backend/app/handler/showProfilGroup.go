	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodGet:
			...
			if groupInfo.AvatarPath != "" {
				groupInfo.AvatarPath, err = helper.EncodeImageToBase64("./app/static/avatarImage/" + groupInfo.AvatarPath)
				if err != nil {
			...
			for i, userNot := range userNotInGroup {
				if userNot.User.AvatarPath != "" {
					encodedAvatar, err := helper.EncodeImageToBase64("./app/static/avatarImage/" + userNot.User.AvatarPath)
					if err != nil {
						log.Println("enable to encode image avatar", err.Error())
						continue
					}
			...
			for i, member := range listMember {
				if member.AvatarPath != "" {
					encodedAvatar, err := helper.EncodeImageToBase64("./app/static/avatarImage/" + member.AvatarPath)
					if err != nil {
						log.Println("enable to encode image avatar", err.Error())
						continue
					}
